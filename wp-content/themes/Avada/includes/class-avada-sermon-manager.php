...
class Avada_Sermon_Manager {
	...
	public function get_sermon_content( $archive = false ) {
		...
		ob_start(); ?>
		<p>
			<?php printf( esc_attr__( 'Date: %s', 'Avada' ), esc_attr( $date ) ); ?>
		...
		<?php if ( $archive ) : ?>
			...
			<?php if ( isset( $sermonoptions['archive_player'] ) ) : ?>
				<div class="wpfc_sermon cf">
					<?php
					if ( is_object( $sermon_manager_template ) ) {
						echo wp_kses_post( $sermon_manager_template->wpfc_sermon_media() );
					} else {
						wpfc_sermon_files();
					}
					?>
		...
		<?php if ( ! $archive ) : ?>
			...
			if ( is_object( $sermon_manager_template ) ) {
				...
				echo wp_kses_post( $sermon_manager_template->wpfc_sermon_attachments() );
			} else {
				wpfc_sermon_files();
	...
	public function render_wpfc_sorting() {
		if ( class_exists( 'Sermon_Manager_Template_Tags' ) ) {
			$sermon_manager_template = new Sermon_Manager_Template_Tags();
			echo wp_kses_post( $sermon_manager_template->render_wpfc_sorting() );
		} else {
			render_wpfc_sorting();
		}
	}
