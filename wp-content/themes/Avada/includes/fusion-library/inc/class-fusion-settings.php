		...
		switch ( $type ) {
			...
			default:
				if ( '' !== $setting_value ) {
					...
					$setting_description = sprintf( __( '  Currently no default selected. Can be set globally from the <a %1s$>%2$s</a>.', 'Avada' ), 'href="' . $this->get_setting_link( $setting, $subset ) . '" target="_blank" rel="noopener noreferrer"', apply_filters( 'fusion_options_label', esc_attr__( 'Element Options', 'Avada' ) ) );
				}
				break;
		}
