...
class PyreThemeFrameworkMetaboxes {
	...
	function admin_script_loader() {
		...
		if ( is_admin() && ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) ) ) {
			...
			wp_enqueue_style(
				...
				FUSION_LIBRARY_URL . '/inc/fusion-redux/fusionredux-framework/FusionReduxCore/inc/fields/slider/vendor/nouislider/fusionredux.jquery.nouislider.css',
				array(),
				'5.0.0',
				'all'
			);
	...
	public function color( $id, $label, $desc = '', $alpha = false, $dependency = array(), $default = '' ) {
		...
		<div class="pyre_metabox_field">
			...
			<?php echo $this->dependency( $dependency ); ?>
			<div class="pyre_desc">
				<label for="pyre_<?php echo esc_attr( $id ); ?>"><?php echo wp_kses_post( $label ); ?></label>
				<?php if ( $desc ) : ?>
					<?php // @codingStandardsIgnoreLine ?>
					<p><?php echo $desc; ?></p>
				<?php endif; ?>
	...
	public function dimension( $ids, $label, $desc = '', $dependency = array() ) {
		...
		<div class="pyre_metabox_field">
			...
			<?php echo $this->dependency( $dependency ); ?>
			<div class="pyre_desc">
				<label for="pyre_<?php echo esc_attr( $ids[0] ); ?>"><?php echo wp_kses_post( $label ); ?></label>
				<?php if ( $desc ) : ?>
					<p><?php echo wp_kses_post( $desc ); ?></p>
				<?php endif; ?>
			</div>
	...
	public function textarea( $id, $label, $desc = '', $default = '', $dependency = array() ) {
		...
		<div class="pyre_metabox_field">
			...
			<div class="pyre_desc">
				<?php // @codingStandardsIgnoreLine ?>
				<label for="pyre_<?php echo esc_attr( $id ); ?>"><?php echo $label; ?></label>
				<?php if ( $desc ) : ?>
					<p><?php echo wp_kses_post( $desc ); ?></p>
				<?php endif; ?>
			</div>
