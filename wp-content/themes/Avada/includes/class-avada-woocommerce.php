...
class Avada_Woocommerce {
	...
	public function __construct() {
		...
		if ( version_compare( self::get_wc_version(), '2.7', '<' ) ) {
			...
			if (  '1' === Avada()->settings->get( 'disable_woo_gallery' ) ) {
				...
				add_filter( 'woocommerce_single_product_image_thumbnail_html', array( $this, 'single_product_image_thumbnail_html' ), 2, 10 );
			}
		}
	...
	public function before_container() {
		...
		<div class="woocommerce-container">
			<div id="content"<?php echo wp_kses_post( $content_class . ' ' . $content_css ); ?>>
		<?php
	}
	...
	public function show_product_loop_outofstock_flash() {
		get_template_part( 'templates/wc-product-loop-outofstick-flash' );
	}
	...
	public function template_loop_add_to_cart( $args = array() ) {
		...
		if ( $product && ( ( $product->is_purchasable() && $product->is_in_stock() ) || $product->is_type( 'external' ) ) ) {
			...
			if ( version_compare( self::get_wc_version(), '2.5', '>=' ) ) {
				...
				$product_type = 'simple';
				if ( method_exists( $product, 'get_type' ) && is_callable( $product, 'get_type' ) ) {
	...
	public function single_product_image_thumbnail_html( $html, $attachment_id ) {
		...
		$html = str_replace( '</figure>', '<a class="avada-product-gallery-lightbox-trigger" href="' . esc_url( $full_size_image[0] ) . '" data-rel="iLightbox' . $gallery . '"></a></figure>', $html );
	...
	public function order_button_html( $html ) {
		return str_replace( 'class="button', 'class="button fusion-button button-default fusion-button-dfault-size', $html );
	}
	...
	public function before_cart_table( $args ) {
		...
		<div class="woocommerce-content-box full-width clearfix">
			<?php if ( 1 == $woocommerce->cart->get_cart_contents_count() ) : ?>
				...
				<h2><?php printf( esc_attr__( 'You Have %d Item In Your Cart', 'Avada' ), $woocommerce->cart->get_cart_contents_count() ); ?></h2>
			<?php else : ?>
				<?php // @codingStandardsIgnoreLine ?>
				<h2><?php printf( esc_attr__( 'You Have %d Items In Your Cart', 'Avada' ), $woocommerce->cart->get_cart_contents_count() ); ?></h2>
			<?php endif; ?>
			<?php
	}
