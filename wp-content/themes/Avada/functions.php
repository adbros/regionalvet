 ...
 */
include_once wp_normalize_path( get_template_directory() . '/includes/fusion-library/fusion-library.php' );
...
function avada_login_fail( $url = '', $raw_url = '', $user = '' ) {
	if ( ! is_account_page() ) {
		...
		if ( isset( $_SERVER ) && isset( $_SERVER['HTTP_REFERER'] ) && esc_url_raw( wp_unslash( $_SERVER['HTTP_REFERER'] ) ) ) {
			...
			$parsed_url    = ( isset( $_SERVER['SERVER_PORT'] ) ) ? wp_parse_url( esc_url_raw( wp_unslash( $_SERVER['SERVER_PORT'] ) ) ) : array( 'host' => '80' );
			...
			if ( ! empty( $referer ) && ! strstr( $referer, 'wp-login' ) && ! strstr( $referer, 'wp-admin' ) ) {
				if ( is_wp_error( $user ) ) {
					...
					wp_safe_redirect( add_query_arg( array( 'login' => 'failed' ), $referer ) );
				} else {
					wp_safe_redirect( $referer );
				}
				exit;
...
function woocommerce_product_archive_description() {
	if ( is_post_type_archive( 'product' ) && 0 == get_query_var( 'paged' ) ) {
		...
		if ( $shop_page ) {
			...
			if ( $description ) {
				echo '<div class="post-content">' . wp_kses_post( $description ) . '</div>';
			}
		}
	}
}
...
function avada_reset_all_cache() {
	...
	$dynamic_css->reset_all_caches();
}
add_action( 'after_switch_theme', 'avada_reset_all_cache' );
