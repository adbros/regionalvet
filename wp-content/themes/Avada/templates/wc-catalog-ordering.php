...
<div class="catalog-ordering clearfix">
	...
	<ul class="sort-count order-dropdown">
		<li>
			<span class="current-li"><a aria-haspopup="true"><?php echo wp_kses_post( sprintf( __( 'Show <strong>%s Products</strong>', 'Avada' ), (int) $per_page ) ); ?></a></span>
			<ul>
				<li class="<?php echo ( $pc == $per_page ) ? 'current' : ''; ?>">
					<a href="<?php echo esc_url_raw( fusion_add_url_parameter( $query_string, 'product_count', $per_page ) ); ?>"><?php echo wp_kses_post( sprintf( __( 'Show <strong>%s Products</strong>', 'Avada' ), (int) $per_page ) ); ?></a>
				</li>
				<li class="<?php echo ( $pc == $per_page * 2 ) ? 'current' : ''; ?>">
					<a href="<?php echo esc_url_raw( fusion_add_url_parameter( $query_string, 'product_count', $per_page * 2 ) ); ?>"><?php echo wp_kses_post( sprintf( __( 'Show <strong>%s Products</strong>', 'Avada' ), (int) $per_page * 2 ) ); ?></a>
				</li>
				<li class="<?php echo ( $pc == $per_page * 3 ) ? 'current' : ''; ?>">
					<a href="<?php echo esc_url_raw( fusion_add_url_parameter( $query_string, 'product_count', $per_page * 3 ) ); ?>"><?php echo wp_kses_post( sprintf( __( 'Show <strong>%s Products</strong>', 'Avada' ), (int) $per_page * 3 ) ); ?></a>
				</li>
			</ul>
		</li>
	</ul>
