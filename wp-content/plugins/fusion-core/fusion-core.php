...
if ( ! class_exists( 'FusionCore_Plugin' ) ) {
	...
	class FusionCore_Plugin {
		...
		private function __construct() {
			...
			add_action( 'setup_theme', array( $this, 'register_post_types' ) );
		...
		public function portfolio_single_template( $single_post_template ) {
			global $post;
			$single_portfolio_template = wp_normalize_path( dirname( __FILE__ ) . '/templates/single-avada_portfolio.php' );

			// Checks if the single post is portfolio.
			if ( 'avada_portfolio' === $post->post_type && file_exists( $single_portfolio_template ) ) {
...
function setup_fusion_slider() {
	...
	if ( ! class_exists( 'Fusion_Settings' ) || '0' !== $fusion_settings->get( 'status_fusion_slider' ) ) {
		include_once wp_normalize_path( FUSION_CORE_PATH . '/fusion-slider/class-fusion-slider.php' );
	}
}
// Setup Fusion Slider.
add_action( 'after_setup_theme', 'setup_fusion_slider' );
...
register_activation_hook( __FILE__, 'fusion_core_enable_elements' );
// Reset patcher on activation/deactivation.
register_activation_hook( __FILE__, 'fusion_core_reset_patcher_counter' );
register_deactivation_hook( __FILE__, 'fusion_core_reset_patcher_counter' );
