...
if ( ! defined( 'FUSION_BUILDER_VERSION' ) ) {
	define( 'FUSION_BUILDER_VERSION', '1.1.1' );
}
// Plugin Folder Path.
...
include_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . '/inc/fusion-library/fusion-library.php' );
...
if ( ! class_exists( 'FusionBuilder' ) ) :
	...
	class FusionBuilder {
		...
		public static function activation() {
			...
			// Delete compiled JS.
			Fusion_Dynamic_JS_File::delete_compiled_js();
		}
		...
		public function init_global_vars() {
			...
			$dynamic_css = new Fusion_Builder_Dynamic_CSS();
		...
		public function shortcode_scripts() {
			...
			wp_enqueue_style( 'fusion-builder-animations', FUSION_BUILDER_PLUGIN_URL . 'animations.css', array(), FUSION_BUILDER_VERSION );
			wp_enqueue_style( 'fusion-builder-ilightbox', FUSION_BUILDER_PLUGIN_URL . 'ilightbox.css', array(), FUSION_BUILDER_VERSION );
			...
			// Font-awesome CSS.
			wp_enqueue_style( 'fusion-font-awesome', FUSION_BUILDER_PLUGIN_URL . 'inc/fusion-library/assets/fonts/fontawesome/font-awesome.css', array(), FUSION_BUILDER_VERSION );
		...
		public function register_scripts() {
			...
			Fusion_Dynamic_JS::register_script(
				'fusion-animations',
				FUSION_BUILDER_PLUGIN_URL . '/assets/js/min/general/fusion-animations.js',
			...
			Fusion_Dynamic_JS::register_script(
				'jquery-count-to',
				FUSION_BUILDER_PLUGIN_DIR . '/assets/js/min/library/jquery.countTo.js',
				array( 'jquery' ),
				'1',
				true
			);
			Fusion_Dynamic_JS::register_script(
				'jquery-count-down',
				FUSION_BUILDER_PLUGIN_DIR . '/assets/js/min/library/jquery.countdown.js',
				array( 'jquery' ),
				'1.0',
				true
			);
			...
			Fusion_Dynamic_JS::register_script(
				'fusion-video',
				FUSION_BUILDER_PLUGIN_DIR . '/assets/js/min/general/fusion-video.js',
		...
		public function admin_scripts( $hook ) {
			...
			if ( ( 'post.php' === $pagenow || 'post-new.php' === $pagenow ) && post_type_supports( $typenow, 'editor' ) ) {
				...
				// FontAwesome.
				wp_enqueue_style( 'fusion-font-awesome', FUSION_BUILDER_PLUGIN_URL . 'inc/fusion-library/assets/fonts/fontawesome/font-awesome.css', false, FUSION_BUILDER_VERSION, 'all' );
		...
		private function includes() {
			...
			// Helper functions.
			require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/helpers.php' );
			require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/class-fusion-builder-options.php' );
			require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/class-fusion-builder-dynamic-css.php' );

			require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/class-fusion-builder-element.php' );
			Fusion_Builder_Options::get_instance();
			new Fusion_Builder_Dynamic_CSS();
			...
			require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/shortcodes.php' );

			// Page layouts.
			require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/layouts.php' );

			if ( is_admin() ) {
				// Importer/Exporter.
				require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/importer/importer.php' );
				// Builder underscores templates.
				require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/templates.php' );
				// Settings.
				require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . 'inc/fusion-builder-admin.php' );

				require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . '/inc/class-fusion-builder-options-panel.php' );
			...
			if ( class_exists( 'WooCommerce' ) ) {
				require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . '/inc/woocommerce/woo-config.php' );
			}
		}
...
function fusion_builder_activate() {
	...
	require_once wp_normalize_path( FUSION_BUILDER_PLUGIN_DIR . '/inc/dynamic-css/dynamic_css.php' );
