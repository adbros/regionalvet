				...
				<?php if ( ! empty( $messages ) ) : ?>
					<?php foreach ( $messages as $message_id => $message ) : ?>
						<?php if ( false !== strpos( $message_id, 'write-permissions-' ) ) : ?>
							<?php continue; ?>
						<?php endif; ?>
						<p class="fusion-patcher-error"><?php echo wp_kses_post( $message ); ?></p>
					<?php endforeach; ?>
				<?php endif; ?>
							...
							<?php foreach ( $available_patches as $key => $patch_id ) :
								...
								<tr class="fusion-patcher-table-head">
									...
									<td class="patch-description">
										...
										<?php endif; ?>
										<?php echo wp_kses_post( $patch_args['description'][0] ); ?>
									</td>
										...
										<?php if ( $can_apply ) : ?>
											<form method="post" action="options.php">
												...
												<?php else : ?>
													...
													<?php if ( $patch_failed ) : ?>
														<a class="button" style="margin-top:10px;font-size:11px;color:#b71c1c;display:block;" href="<?php echo esc_url_raw( admin_url( 'admin.php?page=fusion-support&manually-applied-patch=' . $patch_id ) ); ?>"><?php esc_attr_e( 'Dismiss Notices', 'fusion-builder' ); ?></a>
													<?php endif; ?>
												<?php endif; ?>
											</form>
										<?php else : ?>
