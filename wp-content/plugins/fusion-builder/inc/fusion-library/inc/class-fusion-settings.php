...
class Fusion_Settings {
	...
	protected function __construct() {
		...
		// Added in 'after_setup_theme' to make sure any multilingual plugins
		// have already been loaded.
		add_action( 'after_setup_theme', array( $this, 'multilingual_options' ), 7 );
	...
	public function get_default_description( $setting = null, $subset = false, $type = null, $reset = '', $param = '' ) {
		...
		switch ( $type ) {
			...
			case 'showhide':
				// @codingStandardsIgnoreLine
				$setting_value = ( 1 == $setting_value ) ? esc_html__( 'Show', 'fusion-builder' ) : esc_html__( 'Hide', 'fusion-builder' );
				$setting_link  = '<a href="' . $this->get_setting_link( $setting, $subset ) . '" target="_blank" rel="noopener noreferrer">' . $setting_value . '</a>';
				$setting_description = sprintf( esc_html__( '  Default currently set to %s.', 'fusion-builder' ), $setting_link );
				break;
			...
			case 'yesno':
				// @codingStandardsIgnoreLine
				$setting_value = ( 1 == $setting_value ) ? esc_html__( 'Yes', 'fusion-builder' ) : esc_html__( 'No', 'fusion-builder' );
				$setting_link  = '<a href="' . $this->get_setting_link( $setting, $subset ) . '" target="_blank" rel="noopener noreferrer">' . $setting_value . '</a>';
				$setting_description = sprintf( esc_html__( '  Default currently set to %s.', 'fusion-builder' ), $setting_link );
				break;
			...
			case 'menu':
				...
				$setting_value = ( false !== $menu ) ? $menu->name : esc_attr__( 'undefined', 'fusion-builder' );
			...
			case 'child':
				$setting_description = sprintf( esc_html__( '  Leave empty for value set in parent options.  If that is also empty, the %1$s value of %2$s will be used.', 'fusion-builder' ), apply_filters( 'fusion_options_label', esc_attr__( 'Element Options', 'fusion-builder' ) ), $setting_link );
				break;
			...
			default:
				if ( '' !== $setting_value ) {
					...
					$setting_description = sprintf( __( '  Currently no default selected. Can be set globally from the <a %1s$>%2$s</a>.', 'fusion-builder' ), 'href="' . $this->get_setting_link( $setting, $subset ) . '" target="_blank" rel="noopener noreferrer"', apply_filters( 'fusion_options_label', esc_attr__( 'Element Options', 'fusion-builder' ) ) );
				}
				break;
		}

		return '' === $reset ? $setting_description : $setting_description . '  <span class="fusion-builder-default-reset"><a href="#" id="default-' . $reset . '" class="fusion-range-default fusion-hide-from-atts" type="radio" name="' . $reset . '" value="" data-default="' . $this->get( $setting, $subset ) . '">' . esc_attr__( 'Reset to default.', 'fusion-builder' ) . '</a><span>' . esc_attr__( 'Using default value.', 'fusion-builder' ) . '</span></span>';
	}
