...
class Fusion_FusionRedux {
	...
	public function init_fusionredux() {
		...
		if ( ! class_exists( 'FusionRedux' ) ) {
			require_once wp_normalize_path( dirname( __FILE__ ) . '/fusion-redux/fusionredux-framework/fusionredux-framework.php' );
		}

		require_once wp_normalize_path( dirname( __FILE__ ) . '/fusion-redux/validation-functions.php' );
		if ( ! class_exists( 'Fusion_Redux_Custom_Fields' ) ) {
			require_once wp_normalize_path( dirname( __FILE__ ) . '/fusion-redux/class-fusion-redux-addons.php' );
	...
	public function reset_cache() {
		...
		Fusion_Dynamic_JS_File::reset_cached_filenames();
	}
	...
	public function create_field( $field, $section_id = null ) {
		...
		switch ( $field['type'] ) {
			...
			case 'typography':
				...
				$args['select3'] = array( 'allowClear' => false );
	...
	public function enqueue() {
		...
		wp_register_script( 'fusion-redux-custom-js', trailingslashit( FUSION_LIBRARY_URL ) . 'inc/fusion-redux/assets/fusion-redux.js', array( 'jquery' ), time(), true );
	...
	public function dynamic_css() {
		...
		$styles = $wp_filesystem->get_contents( dirname( __FILE__ ) . '/fusion-redux/assets/style.css' );

		if ( ! $styles || empty( $styles ) ) {
			ob_start();
			include wp_normalize_path( dirname( __FILE__ ) . '/fusion-redux/assets/style.css' );
			$styles = ob_get_clean();
		}

		if ( $styles && ! empty( $styles ) ) {

			$themefusion_logo = trailingslashit( FUSION_LIBRARY_URL ) . 'inc/fusion-redux/assets/themefusion_logo_white.png';
			...
			// @codingStandardsIgnoreLine
			echo '<style id="fusion-redux-custom-styles" type="text/css">' . $styles . '</style>';
	...
	public function add_config() {
		...
		$args = array(
			...
			'customizer'           => false,
			'default_show'         => false,
			'templates_path'       => dirname( __FILE__ ) . '/fusion-redux/panel_templates/',
	...
	public function reset_message_l10n() {
		return esc_html__( 'Are you sure? This will reset all saved options to the default theme values. This does not reset them to a demo that you may have imported.', 'fusion-builder' );
	}
	...
	public function reset_section_message_l10n() {
		return esc_html__( 'Are you sure? This will reset all saved options to the default theme values for this section. This does not reset them to a demo that you may have imported.', 'fusion-builder' );
	}
