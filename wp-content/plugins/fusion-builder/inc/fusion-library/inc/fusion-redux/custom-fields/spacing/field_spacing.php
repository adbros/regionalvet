	...
	class FusionReduxFramework_spacing {
		...
		function render() {
			...
			if ( isset( $this->field['all'] ) && $this->field['all'] == true ) {
				echo '<div class="field-spacing-input input-prepend"><span class="add-on"><i class="el el-fullscreen icon-large"></i></span><input type="text" class="fusionredux-spacing-all fusionredux-spacing-input mini ' . $this->field['class'] . '" placeholder="' . __( 'All', 'fusion-builder' ) . '" rel="' . $this->field['id'] . '-all" value="' . $this->value['top'] . '"></div>';
			}
		...
		function enqueue() {
			...
			wp_enqueue_script(
				...
				trailingslashit( FUSION_LIBRARY_URL ) . 'inc/fusion-redux/custom-fields/spacing/field_spacing.js',
			...
			wp_enqueue_style(
				...
				trailingslashit( FUSION_LIBRARY_URL ) . 'inc/fusion-redux/custom-fields/spacing/field_spacing.css',
				array(),
				time(),
				'all'
			);
